...
|---|---|
| **Processes** | XMRig/P2PInfect cmdline signatures, high CPU since start, execution from `/tmp` or `/dev/shm` |
| **Network** | Active connections to known mining pool IPs, stratum ports (3333/4444/5332), IRC ports (6667/6697) |
| **Persistence** | Systemd services with miner keywords or suspicious paths, recently modified crons, `/etc/ld.so.preload` rootkits |
...
| **Logs** | Empty `auth.log` (evidence destruction), SSH brute force (>20 failures from same IP), attack timeline |
